    ...
    <div class="container hidden-print">
      <div class="row">
        ...
        <div class="col-sm-2 col-xs-3">
          ...
          <select class="form-control" name="no_pakai_brg" id="dropPakai" style="width: 100%;">
      ...
      <main>
        ...
        <div class="row">
          <div class="col-sm-12 col-xs-12 table-responsive">
            <table id="rptReturPakaiBarang" class="table table-bordered" border="0" cellspacing="0" cellpadding="0" width="100%">
              <thead>
                <tr>
                  <th class="text-center col-xs-1 col-sm-1">Tgl. Retur</th>
                  <th class="text-center col-xs-2 col-sm-2">No Nota</th>
                  <th class="text-center col-xs-1 col-sm-1">Tgl. Beli</th>
      ...
      $.ajax({
        ...
        {   
          ...
          for (var i = 0; i < data.length; i++)
          {
            ...
            option.text = data[i]['no_pakai_brg']+' - '+data[i]['nopol'];
            select.add(option);
          }
          $('#dropPakai').select2({placeholder: 'Pilih Pemakaian'});
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
          alert('Error get Pakai data');
        }
      });
    }
    function fetchData()
    {
      table = $('#rptReturPakaiBarang').DataTable({
        "info": false,
        "responsive": true,
        "processing": true,
        "serverSide": true,
        ...
        "order": [],
        "ajax": {
          "url": "<?php echo site_url('Datatables/rptReturPakaiBarang')?>",
          "type": "POST",
          "data": function(data)
            {
              data.no_pakai_brg = $('[name="no_pakai_brg"]').val();
