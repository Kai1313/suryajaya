      ...
      <main>
        <div class="row">
          <div class="col-sm-4 col-xs-4 to-details">
            ...
            <h3>Jumlah <span name="jml_klaim" class="chgnum"></span></h3>
          </div>
  ...
  <script>
    ...
    {
      $.ajax({
        url : "<?php echo site_url('Crud/getPrintKlaimSopir/')?>"+key,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
          $('[name="karyawan"]').text('Nama '+data['a'].nama_driver);
          $('[name="keterangan"]').text('Keterangan '+data['a'].ket_klaim);
          $('[name="jml_klaim"]').text('Jumlah '+data['a'].nom_klaim);
          $('[name="data_code"]').text(data['a'].no_klaim);
          $('[name="data_date"]').text(data['a'].tgl_klaim);
