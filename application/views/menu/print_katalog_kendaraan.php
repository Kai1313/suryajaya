  ...
  <script>
    ...
    {
      $.ajax({
        url : "<?php echo site_url('Crud/getPrintKatalog/')?>"+key,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
          ...
          $('[name="data_code"]').text(data['a'].no_katalog);
          $('[name="data_date"]').text(data['a'].tgl_katalog);
          var blankrow = 7-data['b'].length;
          for (var i = 0; i < data['b'].length; i++)
          {
            var $tr = $('<tr>').append(
              $('<td class="text-center">'+data['b'][i]["ket_det"]+'</td>'),
              $('<td class="text-right chgnum">'+data['b'][i]["qty_det"]+'</td>')
              ).appendTo('#tb_content');
          }
          for (var j = 0; j < blankrow; j++)
          {
            var $tr = $('<tr>').append(
              ...
              $('<td class="text-center"><button type="button" class="btn btn-danger btn-sm hidden-print delBtn">X</button></td>')
              ).appendTo('#tb_content');
          }
