  ...
  <script>
    ...
    {
      $.ajax({
        url : "<?php echo site_url('Crud/getPrintPasangBan/')?>"+key,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
          $('[name="nopol"]').text('Nopol '+data['a'].nopol);
          $('[name="bengkel"]').text('Bengkel '+data['a'].bengkel_pemasangan);
          $('[name="data_code"]').text(data['a'].no_pemasangan);
          $('[name="data_date"]').text(data['a'].tgl_pemasangan);
          ...
          for (var i = 0; i < data['b'].length; i++)
          {
            ...
            var $tr = $('<tr>').append(
              ...
              $('<td class="text-right chgnum">'+data['b'][i]["qty_pasang"]+'</td>')
              ).appendTo('#tb_content');
          }
